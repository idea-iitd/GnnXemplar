...
from gnns import GCN  # using our generic GCN from gnns.py
...
def main():
    parser = argparse.ArgumentParser(
        description='Roman Empire Node Classification')
    # Device & logging
    parser.add_argument('--device', type=int, default=0)
    parser.add_argument('--log_steps', type=int, default=10)
    # Hyperparameters (consistent with Polynormer search space, but we are not grid searching)
    parser.add_argument('--lr', type=float, default=0.01,
                        help="Learning rate, choose from {0.001, 0.005, 0.01}")
    parser.add_argument('--epochs', type=int, default=200,
                        help="Epoch limit (default 2500)")
    parser.add_argument('--hidden_channels', type=int, default=64,
                        help="Hidden dimension, choose from {64, 256, 512}")
    parser.add_argument('--num_layers', type=int, default=3,
                        help="Number of GNN layers, choose from 1 to 10")
    ...
    # Load Roman Empire dataset from local files via our load_dataset branch:
    # Here we assume your load_dataset function supports "roman" as the dataset name.
    data, _, _, _ = load_dataset("roman")
    data = data.to(device)

    # Ensure masks are 1D
    data.train_mask = ensure_1d_mask(data.train_mask)
    data.val_mask = ensure_1d_mask(data.val_mask)
    data.test_mask = ensure_1d_mask(data.test_mask)
    ...
    torch.save(model.state_dict(), "data/RomanEmpire/model.pt")
